...
function triggerGeofencedLocate() {
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      try {
        const response = await fetch(API_BASE_URL + '/geo/check', {
          method: 'POST',
...
async function fetchOfficialRTOChallan() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/challan/lookup/${encodeURIComponent(vn)}`);
    if (!response.ok) {
...
async function uploadChallanReceiptForOcr(file) {
  ...
  try {
    const response = await fetch(API_BASE_URL + '/challan/upload-receipt', {
      method: 'POST',
      body: formData
    });
...
async function fetchStateComparison(violationId) {
  try {
    const response = await fetch(`${API_BASE_URL}/rules/compare/${violationId}`);
    if (response.ok) {
      const data = await response.json();
